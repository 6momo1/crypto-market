...
export async function fetchTokenChartData(address: string, client: ApolloClient<any>) {
  let data: {
    date: number
    volumeUSD: string
    totalValueLockedUSD: string
  ...
  try {
    while (!allFound) {
      const { data: chartResData, errors, loading } = await client.query<ChartResults>({
        query: TOKEN_CHART,
        variables: {
          address: address,
