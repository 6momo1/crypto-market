...
const PRICE_CHART = graphql_tag_1.default `
  query tokenHourDatas($startTime: Int!, $address: Bytes!) {
    tokenHourDatas(
      first: 100
...
function fetchCurrentPriceData(address, client) {
    return __awaiter(this, void 0, void 0, function* () {
        const timeWindow = intervals_1.TimeWindow.DAY;
        const utcCurrentTime = dayjs_1.default();
        const startTimestamp = utcCurrentTime.subtract(1, timeWindow).startOf('minute').unix();
        const { data, error } = yield fetchTokenPriceData(address, intervals_1.ONE_MINUTE_SECONDS, startTimestamp, client);
        return { data, error };
    });
}
